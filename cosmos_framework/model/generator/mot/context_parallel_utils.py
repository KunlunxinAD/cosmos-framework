...
def context_parallel_attention(
    ...
    # size, so the returned tower may be shorter *or* longer than the cp-padded
    # query length.  Scattering such a length yields uneven shards (the last cp
    # rank gets fewer tokens), so the gen/und shard would no longer match the
    # seq-sharded residual on that rank.  Pad (or truncate) back to
    # ``q_*_seq_len`` -- the gathered full length captured before attention,
    # which is a multiple of the cp world size -- so the scatter produces even
    # shards on every rank.  NOTE: no directional torch._check here (output
    # length >= query length does NOT hold on the unpad path); the ``==``/``>``
    # branches below intentionally guard the data-dependent length.
    ...
    attn_output_gen_hp = _pad_or_trunc_seq(attn_output_gen_hp, q_gen_seq_len)  # [gen_len,H_local,head_dim]
